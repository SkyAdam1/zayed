...
class UserAuthenticatedMixin:
    ...
    @method_decorator(
        user_passes_test(
            lambda u: not u.is_staff or not u.is_expert,
            login_url="applications_output_url",
        )
    )
    ...
    def get(self, request, id):
        ...
        if self.request.user != obj.user and not self.request.user.is_staff:
            if self.request.user.is_expert:
                ex = [
                    i.expert
                    for i in models.DesignatedExpert.objects.filter(app=obj.app)
                ]
                if self.request.user not in ex:
        ...
        return render(
            request,
            self.template_name,
            context={"applications": obj, "form": self.form_class, "remarks": remarks},
        )
